...
$stmt = $pdo->prepare("SELECT * FROM materials LIMIT :start_from, :records_per_page"); 
...
$total_sql = "SELECT COUNT(*) AS total FROM materials";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Insert new medicine
    if(isset($_POST["material"]) && isset($_POST["quantity"]) && isset($_POST["expiry_date"])) {
        $ingredientName = $_POST["material"];
        $quantity = $_POST["quantity"];
        $expiry_date = $_POST["expiry_date"];

        $stmt = $pdo->prepare("INSERT INTO materials (material, quantity, expiry_date) VALUES (:material, :quantity, :expiry_date)");
        $stmt->bindParam(':material', $ingredientName);
        ...
        $id = $pdo->lastInsertId();
        echo json_encode(array('success' => true, 'id' => $id, 'material' => $ingredientName, 'quantity' => $quantity, 'time' => date('H:i:s'), 'date' => date('Y-m-d'), 'expiry_date' => $expiry_date));
        exit;
    }
    // Delete medicine
...
form.addEventListener('submit', function(e) {
    ...
    const ingredient = document.getElementById('material').value;
    ...
    xhr.onload = function() {
        if (xhr.status === 200) {
            ...
            if (response.success) {
                ...
                cell2.innerHTML = response.material;
                cell3.innerHTML = response.quantity;
                cell4.innerHTML = response.time;
                cell5.innerHTML = response.date;
                cell6.innerHTML = response.expiry_date;
                cell7.innerHTML = '<form method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>"><input type="hidden" name="delete_medicine" value="' + response.id + '"><button type="submit" class="delete-btn">delete</button></form>';
            }
        }
    };
    xhr.send('material=' + ingredient + '&quantity=' + quantity + '&expiry_date=' + expiry_date);
});

        </script>
        <script>
document.getElementById('medicine-table').addEventListener('click', function(e) {
    // Check if the click is on a delete button
    if (e.target && e.target.matches('.delete-btn')) {
        // Get the ID of the item to delete
        const id = e.target.closest('tr').getAttribute('data-medicine-id');
