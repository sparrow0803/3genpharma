...
try {
    ...
    if (!empty($search_date_created)) {
        $stmt = $pdo->prepare("SELECT * FROM genpharma.supply WHERE DATE(date_created) = :search_date_created LIMIT :limit_start, :records_per_page");
        $stmt->bindParam(':search_date_created', $search_date_created, PDO::PARAM_STR);
    } else {
        $stmt = $pdo->prepare("SELECT * FROM genpharma.supply LIMIT :limit_start, :records_per_page");
    }
    ...
    if (!empty($search_date_created)) {
        $stmt_count = $pdo->prepare("SELECT COUNT(*) as total_records FROM genpharma.supply WHERE DATE(date_created) = :search_date_created");
        $stmt_count->bindParam(':search_date_created', $search_date_created, PDO::PARAM_STR);
    } else {
        $stmt_count = $pdo->prepare("SELECT COUNT(*) as total_records FROM genpharma.supply");
    }
    $stmt_count->execute();
...
if (isset($_POST['remove_button'])) {
    try {
        ...
        $stmt_check = $pdo->prepare("SELECT * FROM inventory WHERE medicine_name = :medicine_name");
        $stmt_check->execute(['medicine_name' => $medicineName]);
        $existingMedicine = $stmt_check->fetch(PDO::FETCH_ASSOC);

        if ($existingMedicine) {
            // Update the total if the medicine already exists
            $stmt_update = $pdo->prepare("UPDATE inventory SET total = total + :total WHERE medicine_name = :medicine_name");
            $stmt_update->execute(['total' => $total, 'medicine_name' => $medicineName]);
        } else {
            // Insert the medicine if it does not exist
            $stmt_insert = $pdo->prepare("INSERT INTO inventory (medicine_name, total) VALUES (:medicine_name, :total)");
            $stmt_insert->execute(['medicine_name' => $medicineName, 'total' => $total]);
        }

        // Mark the medicine as sent in the supply table
        $stmt_update_sent = $pdo->prepare("UPDATE supply SET sent = 1 WHERE medicine_name = :medicine_name");
        $stmt_update_sent->execute(['medicine_name' => $medicineName]);

        // Reset the total to zero in the inventory table
        $stmt_reset_total = $pdo->prepare("UPDATE supply SET total = 0 WHERE medicine_name = :medicine_name");
        $stmt_reset_total->execute(['medicine_name' => $medicineName]);
...
<section class="home">
    <div class="userheader"> 
        <h1> <i class='bx bx-group icon'>&nbsp</i> Supply Chain</h1> 
            ...
            <tbody>
                <?php foreach ($medicines as $medicine): ?>
                    <tr>
                        ...
                        <td><?= htmlspecialchars($medicine['total']); ?></td>
                        <td>
                        <form style="margin: 0; padding: 0; box-shadow: 0 0 0;" method="POST" action="">
                                ...
                                <input type="hidden" name="quantity" value="<?= htmlspecialchars($medicine['total']); ?>">
...
?>
    ...
    <?php
                ...
                $pdoLedgerQuery = "SELECT * FROM financial_request WHERE department = 'Supply Chain'";
